...
const StartProjectChild = () => {
    ...
    const [uploadButtonText, setUploadButtonText] = useState<string>("Upload a file");
    ...
    const validateForm = (): boolean => {
        ...
        // Check if either filenames or keyskills is present
        if (!formData.filenames && !formData.keyskills?.trim()) {
            errors.filenames = 'Either job description or file upload is required';
            errors.keyskills = 'Either job description or file upload is required';
        }
    ...
    const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
        ...
        if (!ALLOWED_FILE_TYPES.includes(file?.type)) {
            toast.error("Please upload a valid file (PDF, DOC, DOCX, TXT, WebP, or SVG)");
            return;
        }
        ...
        setUploadButtonText(file?.name || "Upload a file");
    ...
    const submitHireTalentForm = async (values: FormFields): Promise<void> => {
        try {
            if (validateForm()) {
                ...
                formDataToSend.append('currentFormUrl', `${NEXT_PUBLIC_FRONTEND_URL}${currentUrl}`);

                const response = await axios.post(`${NEXT_PUBLIC_BACKEND_URL}/forStartAProject`, formDataToSend, {
    ...
    return (
        <>
            <form
                ...
                id="start_project" data-form-type="start-a-project-form"
                onSubmit={(e) => e.preventDefault()}>
                <div className="col-lg-4">
                    <div className="form-white-wrapper form-group">
                        <input
                            type="text"
                            className={`c-form-control ${errors.username ? 'is-invalid' : ''}`}
                            name="username"
                            id="username2"
                            placeholder="Name*"
                            maxLength={100}
                ...
                <div className="col-lg-12 mb-1">
                    <span className="drop-zone__prompt">Which services are you looking for?</span>
                </div>
                <div className="col-lg-12">
                    <div className="form-white-wrapper form-group">
                        ...
                        {/* {errors.experience && (
                            <span className="invalid-feedback-error">{errors.experience}</span>
                        )} */}
                    </div>
                </div>
                <div className="col-lg-12 mb-1">
                    ...
                    <div className="form-white-wrapper form-group">
                        <span className="wpcf7-form-control-wrap filename">
                            <div className="drop-zone">
                                ...
                                <input style={{ padding: "40px !important" }}
                                    type="file"
                                    name="filenames"
                                    className={`wpcf7-form-control-1 wpcf7-file drop-zone__input  ${formData.keyskills.trim() ? 'disabled-field' : ''}`}
                                    id="myFile_start_project"
                                    accept=".pdf,.doc,.docx,.txt,.webp,.svg,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,text/plain,image/webp,image/svg+xml"
                                    aria-invalid="false"
                                    ref={fileInputRef}
                    ...
                    <div className="form-white-wrapper form-group">
                        ...
                        <span className="wpcf7-form-control-wrap KeySkills">
                            ...
                            <textarea
                                ...
                                cols={40}
                                rows={2}
                                className={`wpcf7-form-control wpcf7-textarea tabing-textarea ${formData.filenames ? 'disabled-field' : ''}`}
                                id="KeySkills_start_project"
                                aria-invalid="false"
                                placeholder="Add links to the job posts or a brief description of the roles and responsibilities"
                                value={formData.keyskills}
